    ...
    <nav class="navigation-bar" v-bind:class="{'open' : open }">
      <div class="container">
        ...
        <ul class="links margin-top-xs-40">
          <router-link :to="{ path: '/most-popular'}" tag="li" v-if="!user.isLoggedIn">
            <a @click="open =false">
              <span class="dripicons-star"></span> MOST POPULAR
            </a>
          </router-link>
          <router-link :to="{ path: '/watched-movies'}" tag="li" v-if="user.isLoggedIn">
            <a>
              <span class="dripicons-camcorder"></span> WATCHED MOVIES
            </a>
          </router-link>
          <router-link :to="{ path: '/watched'}" tag="li" v-if="user.isLoggedIn">
            <a>
              <span class="dripicons-preview"></span> WATCHED
            </a>
          </router-link>
          <router-link :to="{ path: '/rewatch-list'}" tag="li" v-if="user.isLoggedIn">
            <a>
              <span class="dripicons-clockwise"></span> REWATCH LIST
            </a>
          </router-link>
          <router-link :to="{ path: '/watch-list'}" tag="li" v-if="user.isLoggedIn">
            <a>
              <span class="dripicons-preview"></span> WATCH LIST
            </a>
          </router-link>
          <router-link :to="{ path: '/my-shows'}" tag="li" v-if="user.isLoggedIn">
            <a>
          ...
          <li ref="dropdownMenu" class="menu-container" v-bind:class="{ 'open' : dropdown }">
            <a href @click.prevent="dropdown = !dropdown">
              <span class="dripicons-user"></span> Me
            </a>
            <ul class="menu">
              <li>
                <a @click.prevent="logout" href>
                  <span class="dripicons-exit"></span> SIGNOUT
                </a>
              </li>
            </ul>
          </li>
      ...
      <ul class="links auth">
        <router-link :to="{ path: '/login'}" tag="li" v-if="!user.isLoggedIn">
          <a>SIGNIN</a>
        </router-link>
      </ul>
    </nav>
...
export default {
  ...
  computed: {
    ...mapActions(["getCurrentUser", "signUserOut"]),
    ...mapGetters(["user"])
  },
  beforeMount() {
